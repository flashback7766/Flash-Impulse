...
Item {
    ...
    // Latches on the first show: see the Loader below.
    ...
    onShownChanged: {
        if (root.shown) root.everShown = true;
        else root.chosenProvider = "";
    }
    ...
    Rectangle {
        ...
        // hovered control underneath keeps its tooltip floating over the wizard.
        MouseArea { anchors.fill: parent; hoverEnabled: true }

        // The wizard's body is built only once it actually has to show. A setup that
        // already has a key configured never constructs any of it.
        Loader {
            id: contentLoader
    ...
    function saveAndCheck() {
        const key = contentLoader.item?.keyText ?? "";
        if (!root.provider || key.trim().length === 0) return;
        Ai.setProviderKey(root.provider.id, key);
        contentLoader.item.keyText = "";
