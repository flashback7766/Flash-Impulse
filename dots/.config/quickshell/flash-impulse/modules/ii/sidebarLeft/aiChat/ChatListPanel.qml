...
Item {
    ...
    function open(focusSearch) {
        root.everShown = true;
        root.shown = true;
        if (focusSearch) Qt.callLater(() => contentLoader.item?.focusSearch());
    }
    ...
    property string query: (contentLoader.item?.searchText ?? "").trim()
...
`, Directories.aiChats, root.query]
    ...
    Rectangle {
        ...
        // hoverEnabled too, not just the fill: otherwise this only swallows
        // clicks, and a hovered control underneath keeps reporting itself
        // hovered — its tooltip ends up floating over this panel.
        MouseArea { anchors.fill: parent; hoverEnabled: true }

        // Built on first open, like the settings sheet.
        Loader {
            id: contentLoader
            anchors.fill: parent
            anchors.margins: 12
