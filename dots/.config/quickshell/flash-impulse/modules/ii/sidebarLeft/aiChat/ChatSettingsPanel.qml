...
Item {
    ...
    // Latches on the first open and never clears: see the Loader below. Once the
    // body exists there is nothing to gain by dropping it again mid-session —
    // closing the sidebar for a while takes the whole thing with it anyway.
    ...
    Rectangle {
        ...
        // The sheet's body is built the first time it's opened rather than with the
        // sidebar. It is a lot of UI — every provider, every model, every persona —
        // for something most sessions never open, and it cost around 36 MB of the
        // sidebar's memory just sitting there hidden.
        Loader {
            id: contentLoader
            anchors.fill: parent
            anchors.margins: 12
